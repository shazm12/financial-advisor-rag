...
export default function Home() {
  ...
  useGSAP(() => {
    if (showFileUpload && fileUploadContainerRef.current) {
      gsap.from(fileUploadContainerRef.current, {
        opacity: 0,
        y: 20,
        duration: 0.5,
        ease: "power2.out"
  ...
  const handleFileUpload = async (file: File) => {
    ...
    if (response.success) {
      toast.success(response.description || "File Upload Successful", {
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center p-4">
      ...
      <div
        ref={imgContainerRef}
        className="flex flex-row gap-4 items-center justify-center w-full mb-4"
      >
        <AnimatedText
          delay={5}
          text="Financial wisdom, personalized for you"
          className="mb-0"
        />
      ...
      {/* Progress Indicator */}
      {showFileUpload && (
        <div ref={fileUploadContainerRef} className="flex flex-col mt-4 justify-center items-center w-full opacity-100">
          <div className="flex items-center gap-2 mb-8">
            ...
            <div
              className={`w-20 h-1 ${
                currentStep === "chat" ? "bg-blue-600" : "bg-slate-300"
              }`}
            />
            <div
              className={`w-3 h-3 rounded-full ${
